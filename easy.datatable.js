...
var DataTable = {
  ...
  load:function(tableid, easydataParams) {
    ...
    if (!DataTable.cacheInit[tableid]) {
      ...
      if (this.Validate.integer.test(easydataParams.row)) {
        ...
        if (!this.Validate.integer.test(defRow)) {
    ...
    $.post(dataForm.attr("action"), postParam, function(data) {
      ...
      if (valueObject) {
        ...
        DataTable.cacheData[tableid].maxPage = Math.floor((DataTable.cacheData[tableid].totalCount - 1) / DataTable.cacheData[tableid].rowPerPage + 1);
        ...
        DataTable.cacheData[tableid].maxPage = Math.floor((parseInt(data.totalCount) - 1) / parseInt(data.rowPerPage) + 1);
  ...
  gopage:function(tableid) {
    ...
    if (this.cacheData[tableid]["pageNo"] != pagenoEle.val() && DataTable.Validate.integer.test(pagenoEle.val())) {
      this.load(tableid);
    }
  },
